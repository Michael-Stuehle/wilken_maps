...
module.exports = {
    ...
    checkPasswordForUser: function(user, enteredPassword, callback){
        checkIsConnected(function(){
            setTimeout(function(){
                var sql = "SELECT user.password, user.salt from user where user.name = ?";
    ...
    checkPasswordForUserPasswordVergessen: function(user, enteredPassword, callback){
        checkIsConnected(function(){
            setTimeout(function(){
                var sql = "SELECT user.1malPasswort password, user.salt from user where user.name = ? and TIMESTAMPDIFF(HOUR, 1malPasswortAblauf, DATE_ADD(NOW(), INTERVAL 1 HOUR)) = 0";
    ...
    set1malPasswort: function(user, password, callback){
        checkIsConnected(function(){
            getSalt(user, function(salt){
                ...
                var sql = "Update user set user.1malPasswort = ?, 1malPasswortAblauf = DATE_ADD(NOW(), INTERVAL 1 HOUR) where user.name = ?";
                
                con.query(sql, [final, user], function (err) {
                    if (err) {
                ...
                connection.beginTransaction(function(err){
                    ...
                    for (let index = 0; index < alteEinstellungen.length; index++) {
                        ...
                        getEinstellungIdFromUser(user, function(id){
                            con.query(sql, [newElement.value, id], function(err){
                                if (err) {
                                    logger.logError(err);
                                    connection.rollback(function (err){
                                        logger.logError(err);
    ...
    changePassword: function(user, newPassword, callback){
        checkIsConnected(function(){
            getSalt(user, function(salt){
                var sql = "UPDATE user set password = ? WHERE user.name = ?";
                con.query(sql, [PBKDF2(newPassword, salt), user], function (err, result, fields) {
                    if (err) {
                        return callback(false);
                    }
                    else{
    ...
    isUserVerified: function(username, callback){
        checkIsConnected(function(){
            var sql = "SELECT isVerified FROM user WHERE user.name = ?";
    ...
    verifieUserWithToken: function(username, token, callback){
        checkIsConnected(function(){
            var sql = "Update user set isVerified = 1 where user.name = ? AND user.verificationToken = ? AND user.isVerified = 0" + 
                " AND verificationTokenExpires < DATE_ADD(NOW(), INTERVAL 1 DAY)";
                
            con.query(sql, [username, token], function (err, result, fields) {
                if (err) {
                    logger.log(err);
                    return;
                }
    ...
    getUserRaum: function(user, callback){
        checkIsConnected(function(){
            ...
            con.query(sql, [getNameFromEmail(user)], function (err, result, fields) {
                if (err) {
                    logger.log(err);
                    return;
                }
...
var getEinstellungIdFromUser= function(user, callback){
    checkIsConnected(function(){
        con.query('select einstellungen_id from user where user.name=?', [user], function(err, result){
            if (err) {
                logger.logError(err);
            }
...
var getAllProcedures = function(callback){
    checkIsConnected(function(){
        ...
        con.query(sql, [dbName], function (err, result, fields) {
            if (err) {
                logger.log(err);
                return;
            }
...
var getPermissionsUser = function(user, callback){
    checkIsConnected(function(){
        var sql = "SELECT permissions FROM rank, user where user.name = ? and rank.name = user.rank";
        con.query(sql, [user], function (err, result, fields) {
            if (err){
                logger.log( err);
                return;
